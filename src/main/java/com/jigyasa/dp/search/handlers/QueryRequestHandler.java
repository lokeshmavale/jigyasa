...
import com.jigyasa.dp.search.query.PerRequestSearcher;
...
public class QueryRequestHandler extends RequestHandlerBase<QueryRequest, QueryResponse> {
    ...
    public void internalHandle(QueryRequest req, StreamObserver<QueryResponse> observer) {
        ...
        try (var lease = helpers.indexSearcherManager().leaseSearcher();
             var prs = new PerRequestSearcher(lease, req.getTimeoutMs())) {

            IndexSearcher searcher = prs.searcher();
            ...
            if (queryBuilder.isHybrid(req)) {
                ...
                topDocs = hybridExecutor.execute(searcher, context, numHits);
            } else {
                ...
                if (hasFacets && !(query instanceof MatchAllDocsQuery)) {
                    var facetsResult = executor.executeWithFacets(searcher, query, numHits, sort,
                            useSearchAfter ? req.getSearchAfter() : null);
                    topDocs = facetsResult.topDocs();
                    fc = facetsResult.facetsCollector();
                } else {
                    topDocs = executor.execute(searcher, query, numHits, sort,
                            useSearchAfter ? req.getSearchAfter() : null);
                }
            }

            boolean timedOut = prs.timedOut();

            // Skip facets on timeout — they'd be incomplete and misleading
            Map<String, FacetResult> facets = Collections.emptyMap();
            if (hasFacets && !timedOut) {
                facets = facetExecutor.compute(searcher, fc, schema, req.getFacetsList());
            }

            if (timedOut) {
                log.warn("Query timed out: collection={} query_type={} timeout_ms={} hits_collected={}",
                        req.getCollection(),
                        context.request().getTextQuery().isEmpty() ? "filter" : "text",
                        req.getTimeoutMs(),
                        topDocs.scoreDocs.length);
            }

            QueryResponse response = responseBuilder.build(topDocs, searcher, schema,
                    req.getIncludeSource(), offset, topK, req.getMinScore(),
                    req.getSourceFieldsList(), facets, timedOut);
